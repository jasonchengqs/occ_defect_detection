...
#     def __init__(self, pickle_path, transform, batch_size, phase):
#         self.transform = transform
#         self.data_set = ImageFolderPickle(pickle_path, True, transform=transform)
#         self.data_loader = DataLoader(self.data_set, batch_size=batch_size, shuffle=True, num_workers=4)
...
class STPM(pl.LightningModule):
    ...
    def train_dataloader(self):
        image_datasets = ImageFolderPickle(args.dataset_path, True, transform=self.train_transforms, sample_ratio=args.sample_ratio)
    ...
    def test_dataloader(self):
        image_datasets = ImageFolderPickle(args.dataset_path, False, transform=self.test_transforms)
        # image_datasets = MVTecDataset(root=os.path.join(args.dataset_path,args.category), transform=self.data_transforms, gt_transform=self.gt_transforms, phase='test')
        test_loader = DataLoader(image_datasets, batch_size=1, shuffle=False, num_workers=0) #, pin_memory=True) # only work on batch_size=1, now.
        return test_loader
    ...
    def training_epoch_end(self, outputs): 
        ...
        print('selecting N=',int(total_embeddings.shape[0]*args.coreset_sampling_ratio))
    ...
    def test_step(self, batch, batch_idx): # Nearest Neighbour Search
        ...
        self.gt_list_img_lvl.append(y[0])
        self.pred_list_img_lvl.append(score)
        self.img_path_list.extend(f)
        # save images
    ...
    def test_epoch_end(self, outputs):
        ...
        msg = f'{dt_str}-result: img_auc={img_auc}'
        log_to_txt(dir, msg)
        # anomaly_list = []
        # normal_list = []
