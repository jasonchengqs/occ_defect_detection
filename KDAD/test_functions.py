...
def localization_test(model, vgg, test_dataloader, ground_truth, config, return_maps=False):
    ...
    if return_maps:
        maps = compute_localization_map(grad)
        maps = einops.rearrange(maps, 'm n h w -> n h (m w)')
        maps = einops.repeat(maps, 'n h w -> n h w c', c=3)
        imgs = []
        for i, f in enumerate(test_dataloader.dataset.x):
            x = Image.open(f).convert('RGB')
            x = test_dataloader.dataset.transform_size(x)
            imgs.append(x)
        return imgs, maps
...
def compute_localization_map(grad):
    thresholds = [0.3, 0.5]
    grad_maps = []
    for threshold in thresholds:
        grad_t = 1.0 * (grad >= threshold)
        grad_t = morphological_process(grad_t)
        grad_maps.append(grad_t)
    return grad_maps