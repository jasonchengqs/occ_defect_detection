...
# devices = torch.cuda.get_device_properties(1)
# print(devices)
...
def main():
    ...
    parser.add_argument('--roc',  type=eval, choices=[True, False], default=True)
    args = parser.parse_args()
    args.save_dir = './results' + '/' + args.obj + '/seed_{}/'.format(args.seed)
    ...
    # test_dataset = EMDCDataset(args.data_path, phase='train', sample_size=1000, resize=args.img_size, cropsize=args.img_size)
    test_dataset = EMDCDataset(args.test_data_path, phase='test', sample_size=10, resize=args.img_size, cropsize=args.img_size)
    ...
    save_dir = args.save_dir + '/' + 'test_pics_{:.4f}'.format(threshold)
    os.makedirs(save_dir, exist_ok=True)
    if args.roc:
        plt.savefig(os.path.join(save_dir, args.obj + '_roc_curve.png'), dpi=100)

    save_recon_fig(args, test_imgs, recon_imgs, save_dir)
...
def save_recon_fig(args, test_imgs, recon_imgs, save_dir):
