...
def plt_fig(test_img, scores, img_scores, gt_f_list, threshold, cls_threshold, save_dir, class_name):
    num = len(scores)
    vmax = scores.max() * 255.
    vmin = scores.min() * 255.
    ...
    for i in range(num):
        ...
        fig_img.savefig(os.path.join(save_dir, f'{f}'.format(i)), dpi=300, bbox_inches='tight')
        plt.close()
